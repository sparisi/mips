function [Jr, D_r_Jr] = executeTPoint(domain, t_point, theta_iter, D_t_theta, ...
    D_t_theta_iter, D_rho_theta_iter, t, rho, rho_point, ind_type, beta)
% EXECUTETPOINT Given some free parameters RHO_POINT and a point T_POINT 
% from the generative space, it executes an episode using the parameters 
% generated by THETA_ITER(T_POINT,RHO_POINT). After collecting the sample,
% it calculates the corresponding return JR and gradient D_R_JR.
...
for j = 1 : dim_rho
    ...
    D_r_Dtheta_i = double(subs(D_r_Dtheta, [rho, t], [rho_point, t_point]));
    D_r_Jr(j) = D_r_Jr(j) + ...
        D_rho_L(j) * V + ...
