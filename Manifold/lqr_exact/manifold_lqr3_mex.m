...
n_k = length(k1_opt);
points = samplePoints([0,0]',[1,1]',n_k,useSimplex,1);
p_t1 = points(1,:);
p_t2 = points(2,:);
x_axis_manual = (1:n_k)/n_k;
x_axis_manual_learned = (1:size(points,2))/size(points,2);
...
for j = 1 : size(rho_history,1)
    ...
    subplot(1,3,1,'align'), hold all
    plot(x_axis_manual,k1_opt,'g')
    plot(x_axis_manual_learned,theta_learned(1,:),'-.g')
    axis([0,1,-1,0]), axis manual
    title('\theta_1')
    
    subplot(1,3,2,'align'), hold all
    plot(x_axis_manual,k2_opt,'r')
    plot(x_axis_manual_learned,theta_learned(2,:),'-.r')
    axis([0,1,-1,0]), axis manual
    title('\theta_2')

    subplot(1,3,3,'align'), hold all
    plot(x_axis_manual,k3_opt,'b')
    plot(x_axis_manual_learned,theta_learned(3,:),'-.b')
    axis([0,1,-1,0]), axis manual
    title('\theta_3')
