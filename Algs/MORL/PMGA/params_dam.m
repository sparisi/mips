...
if num_obj == 2
    ...
    theta(1) = sym('66 - rho1*t1^2 + (-16 + rho1)*t1');
    theta(2) = sym('-105 - rho2*t1^2 + (20 + rho2)*t1');
    theta(3) = sym('18 - rho3*t1^2 + (-16 + rho3)*t1');
    theta(4) = sym('-23 - rho4*t1^2 + (53 + rho4)*t1');
    theta(5) = sym('39 - rho5*t1^2 + (121 + rho5)*t1');
    theta(6) = sym('0.01 - rho6*t1^2 + (0.1 + rho6)*t1');
